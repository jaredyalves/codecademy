  ...
  switch (action.type) {
    case 'board/setBoard':
      ...
      action.payload.forEach((element, index) => setState.push({
        id: index, contents: element, visible: false, matched: false,
      }));
      return setState;
    case 'board/flipCard':
      let flipState = [...state];
...
export const setBoard = () => {
  ...
  return {
    type: 'board/setBoard', payload: words,
  };
};

export const flipCard = (id) => {
  return {
    type: 'board/flipCard', payload: id,
  };
};
...
export const selectVisibleIDs = state => state.board.filter(
    card => card.visible).map(card => card.id);

export const selectMatchedIDs = state => state.board.filter(
    card => card.matched).map(card => card.id);